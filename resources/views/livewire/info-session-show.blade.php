<div>
    @canany(['edit others info_session', 'edit own info_session', 'delete own info_session', 'delete other info_session'])
        <div class="flex flex-row items-center gap-2 mb-4 w-full justify-end">
            <x-filament::button icon="heroicon-o-pencil" color="primary" tag="a"
                                href="{{route('info_session.edit', $info_session->id)}}">
                Modifier
            </x-filament::button>
            <x-filament::icon-button icon="heroicon-o-trash" color="danger" size="lg"
                                     tooltip="Supprimer" wire:click="delete"
            />
        </div>
    @endcanany
